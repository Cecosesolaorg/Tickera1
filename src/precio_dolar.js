...
async function addTasaBCVTicket() {
    try {
        let rate = tasaBCV;
        if (!rate || rate === 0) {
            showStatus('Obteniendo tasa BCV...', 'info');
            rate = await fetchTasaBCV();
        }

        if (!rate) {
            showStatus('No se pudo obtener la tasa BCV', 'error');
            return;
        }

        const id = `bcv-${new Date().getTime()}`;
        allCards.unshift({
            id: id,
            name: "TASA BCV",
        ...
        selectedIds.add(id);
        if (searchBox) searchBox.value = ''; // Limpiar búsqueda para ver el nuevo ticket
        renderGrid();
        showStatus('Ticket de Tasa BCV agregado', 'success');
        
        // Scroll to top to see the new ticket
        window.scrollTo({ top: 0, behavior: 'smooth' });
    } catch (error) {
        console.error('Error adding BCV ticket:', error);
        showStatus('Error al agregar ticket de tasa', 'error');
    }
}

// Global exposure
window.fetchTasaBCV = fetchTasaBCV;
window.updateBCVManual = updateBCVManual;
window.injectHistoryModal = injectHistoryModal;
window.openHistoryModal = openHistoryModal;
window.addTasaBCVTicket = addTasaBCVTicket;
